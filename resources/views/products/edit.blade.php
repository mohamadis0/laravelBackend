    ...
    <div class="flex flex-col items-center justify-center mt-10">
        <form action="{{ route('products.update', $product->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div id="featureDropdown" class="mb-3">
                ...
                <span>{{ $product->feature === 'product' ? 'Product' : 'Ingredient' }}</span>
                <input type="text" name="feature" hidden value="{{$product->feature}}">
            </div>
            ...
            <div id="tagsSection" class="mb-3">
                ...
                <select name="tags[]" multiple class="w-full rounded-lg">
                    @foreach ($tags as $tagId => $tagName)
                        <option value="{{ $tagId }}"
                            {{ in_array($tagId, $product->tags->pluck('id')->toArray()) ? 'selected' : '' }}>
                            {{ $tagName }}
                        </option>
                    @endforeach
                ...
                <a class="bg-transparent border-0" style="padding: 5px" href="{{ route('tags.create') }}">
                    <button type="button" class=" bg-[#af0433] text-sm rounded-lg hover:bg-transparent w-full   mt-2 p-1">
                        Add Tags</button>
                </a>
            </div>
            ...
            <div id="addonsSection" class="mb-3">
                ...
                <select name="add[]" multiple class="w-full rounded-lg">
                    @foreach ($addons as $addId => $addName)
                        <option value="{{ $addId }}"
                            {{ in_array($addId, $product->addons->pluck('id')->toArray()) ? 'selected' : '' }}>
                            {{ $addName }}
                        </option>
                    @endforeach
                </select>
            ...
            @endif
            @if (count($ingredients) != 0 && $product->feature!='ingredient')
                <div id="ingredientsSection" class="mb-3">
                        ...
                        <tbody>
                            @foreach ($ingredients as $ingredientId => $ingredientName)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <input type="checkbox" name="ingredient_removable[{{ $ingredientId }}]"
                                            value="1"
                    ...
                    <a class="bg-transparent border-0" style="padding: 5px" href="{{ route('products.create') }}">
                        <button type="button"
                            class=" bg-[#af0433] text-sm rounded-lg hover:bg-transparent w-full   mt-2 p-1"> add
                            ingredient</button>
                    </a>
                </div>
            @endif
